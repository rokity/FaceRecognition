...
from video_renaming import load_dictionary, get_all_video_info
...
def run(init_lr=0.1,
    ...
    if mode == 'flow':
        ...
        i3d.load_state_dict(torch.load('weights/flow_imagenet.pt', map_location=torch.device(process_unit)))
    else:
        i3d = InceptionI3d(400, in_channels=3)
        # i3d.load_state_dict(torch.load('weights/rgb_imagenet.pt'))
        # MODEL_PATH = 'Somemodelname.pth'
        i3d.load_state_dict(torch.load('weights/rgb_imagenet.pt', map_location=torch.device(process_unit)))
    ...
    for data in dataloaders["test"]:
        ...
        prediction = str(torch.argmax(predictions[0]).item())
        label = str(labels[0].item())
        translation += sign_dict[prediction] + ' '
        print('Result: predictions = {}, probs = {}'.format(out_labels[-5:], out_probs[-5:]))
        print('{}: prediction: {}, expected: {}'.format(video_id, sign_dict[prediction], sign_dict[label]))
...
def run_on_tensor(weights, ip_tensor, num_classes, process_unit='cpu'):
    i3d = InceptionI3d(400, in_channels=3)
    # i3d.load_state_dict(torch.load('models/rgb_imagenet.pt'))

    i3d.replace_logits(num_classes)
    i3d.load_state_dict(torch.load(weights, map_location=torch.device(process_unit)))  # nslt_2000_000700.pt nslt_1000_010800 nslt_300_005100.pt(best_results)  nslt_300_005500.pt(results_reported) nslt_2000_011400
    i3d.cuda()
    i3d = nn.DataParallel(i3d)
    i3d.eval()
...
if __name__ == '__main__':
    ...
    #train_split = 'preprocess/nslt_3.json'
